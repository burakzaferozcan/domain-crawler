class Model {
    ...
    for (let i = 0; i < priceList.length; i++) {
      ...
      if (price.new_registration_fee) {
        domains[domain].new_registration_fees.push(
          Model.generatePriceModel(price, "new_registration_fee")
        );
      }

      if (price.transfer_fee) {
        domains[domain].transfer_fees.push(
          Model.generatePriceModel(price, "transfer_fee")
        );
      }

      if (price.renewal_fee) {
        domains[domain].renewal_fees.push(
          Model.generatePriceModel(price, "renewal_fee")
        );
      }
    }
  ...
  static groupByCompany(priceList, paramDomain = {}) {
    ...
    for (let i = 0; i < priceList.length; i++) {
      ...
      if (price.new_registration_fee) {
        companies[company].new_registration_fees.push(
          Model.generatePriceModel(price, "new_registration_fee", paramDomain)
        );
      }

      if (price.transfer_fee) {
        companies[company].transfer_fees.push(
          Model.generatePriceModel(price, "transfer_fee", paramDomain)
        );
      }

      if (price.renewal_fee) {
        companies[company].renewal_fees.push(
          Model.generatePriceModel(price, "renewal_fee", paramDomain)
        );
      }
    }
